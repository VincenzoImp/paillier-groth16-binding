...
interface BallotRecord {
  ...
  voteHash: string;
  nullifier: string;
  validityProofCommitment: string;
  blockNumber: number;
}
...
function canonicalizeTranscript(transcript: Transcript) {
  return {
    params: {
      ...transcript.params,
...
async function loadTranscript(path?: string): Promise<Transcript> {
  ...
  return JSON.parse(await readFile(path, "utf8")) as Transcript;
}
