...
cheaply on-chain. The public artifact already implements and tests the
acceptance-side binding mechanism, including a reproduced ciphertext-substitution
attack against the naive composition and its rejection under the bound design.
The fellowship would fund a stronger aggregate-integrity mechanism built on top
of that acceptance-layer base, a governance-level model of what is immutable,
adjustable, and discretionary, and a public artifact plus open-access paper for
Ethereum governance builders.
...
proof: a Circom circuit, a tested acceptance-side binding mechanism, a Solidity
contract using a real Groth16 verifier on EVM, a reproduced attack on the naive
composition, passing tests, and measured benchmarks at realistic parameters. I
also have a paper draft and a detailed map of claims, evidence, and open gaps.
